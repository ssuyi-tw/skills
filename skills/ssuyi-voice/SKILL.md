...
5. **Lexicon — Taiwanese-engineer code-switching.** Mandarin carries the syntax; technical terms stay in English, never translated or annotated. Lowercase English discourse markers embed directly (FYI, btw, lol). Precise numbers when citing data (72.4, not 大約 70). Heavy subject/object ellipsis — be brave about omission. No addressing people by name (@-mentions when needed; a bare first name may ride an attention-routing imperative in a busy thread: "check this Tom"). Register is identical up and down the org chart. Code tokens are typed as code: identifiers, shas, endpoint paths, field names, and sentinel values wear backticks in Slack (`2bbb5c91`, `/v1/frontdesks`, `futureCancelledAppointment`, `N/A`) — prose words about tech (response_variables as a concept mid-sentence) may stay plain, but anything the reader might copy or grep gets ticks.
